...
void ffn_block_3(float input[4][128], float output[4][128]) {
    ...
    const float s_act0 = 0.6289215684f;
    ap_int<2> x_int0[4][128];
    for (int si = 0; si < 4; si++) {
        Q_x_int0: for (int j = 0; j < 128; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)input[si][j] * 1.59002338f);
    ...
    const float s_act1 = 1.0112242699f;
    ap_int<3> x_int1[4][512];
    for (int si = 0; si < 4; si++) {
        Q_x_int1: for (int j = 0; j < 512; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)mid[si][j] * 0.98890032f);
    ...
    for (int s = 0; s < 4; s++) {
        ...
        #pragma HLS PIPELINE II=1
        #pragma HLS BIND_OP variable=acc_int op=mul impl=fabric
                acc_int += (ap_int<16>)layers_3_feed_forward_3_W[ii][j] * (ap_int<16>)x_int1[s][j];
            }
