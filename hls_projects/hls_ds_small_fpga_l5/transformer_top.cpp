// Delayed Scaling Transformer Top | S=4 D=128 NL=4
...
#pragma HLS INTERFACE ap_ctrl_none port=return
    ...
    input_t input_2d[SEQ_LEN][EMB_DIM];
    for (int si = 0; si < SEQ_LEN; si++)
        for (int j = 0; j < EMB_DIM; j++)
            input_2d[si][j] = input_flat[si * EMB_DIM + j];

    // All intermediate buffers in float (Delayed Scaling dequantizes at each layer)
    ...
    const float s_act_out = 0.1095536575f;
    ap_int<5> out_x_int[SEQ_LEN][EMB_DIM];
    for (int si = 0; si < SEQ_LEN; si++) {
        Q_OUT: for (int j = 0; j < EMB_DIM; j++) {
#pragma HLS PIPELINE II=1
            float _v = roundf((float)x[si][j] * 9.12794719f);
            _v = _v < -15.0f ? -15.0f : (_v > 15.0f ? 15.0f : _v);
            out_x_int[si][j] = (ap_int<5>)_v;
        }
    }
