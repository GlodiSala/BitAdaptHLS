...
void ffn_block_2(float input[4][128], float output[4][128]) {
    ...
    const float s_act0 = 0.6227566004f;
    ap_int<2> x_int0[4][128];
    for (int si = 0; si < 4; si++) {
        Q_x_int0: for (int j = 0; j < 128; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)input[si][j] * 1.60576379f);
    ...
    const float s_act1 = 0.9157854319f;
    ap_int<3> x_int1[4][512];
    for (int si = 0; si < 4; si++) {
        Q_x_int1: for (int j = 0; j < 512; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)mid[si][j] * 1.09195884f);
            _v = _v < -3.0f ? -3.0f : (_v > 3.0f ? 3.0f : _v);
