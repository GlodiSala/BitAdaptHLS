// Delayed Scaling | FFN layer 2 | S=4 D=192 H=768
...
void ffn_block_2(float input[4][192], float output[4][192]) {

    const float s_act0 = 0.5556915998f;
    ap_int<3> x_int0[4][192];
    for (int si = 0; si < 4; si++) {
        Q_x_int0: for (int j = 0; j < 192; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)input[si][j] * 1.79955932f);
    ...
    for (int s = 0; s < 4; s++) {
        for (int ii = 0; ii < 768; ii++) {
            ap_int<16> acc_int = 0;
            MAC_layers_2_feed_forward_0: for (int j = 0; j < 192; j++) {
        #pragma HLS PIPELINE II=1
    ...
    const float s_act1 = 0.9136368632f;
    ap_int<4> x_int1[4][768];
    for (int si = 0; si < 4; si++) {
        Q_x_int1: for (int j = 0; j < 768; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)mid[si][j] * 1.09452676f);
            _v = _v < -3.0f ? -3.0f : (_v > 3.0f ? 3.0f : _v);
    ...
    for (int s = 0; s < 4; s++) {
        for (int ii = 0; ii < 192; ii++) {
            ap_int<17> acc_int = 0;
            MAC_layers_2_feed_forward_3: for (int j = 0; j < 768; j++) {
        #pragma HLS PIPELINE II=1
        #pragma HLS BIND_OP variable=acc_int op=mul impl=fabric
                acc_int += (ap_int<17>)layers_2_feed_forward_3_W[ii][j] * (ap_int<17>)x_int1[s][j];
            }
