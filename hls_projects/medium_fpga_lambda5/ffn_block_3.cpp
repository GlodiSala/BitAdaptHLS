// Delayed Scaling | FFN layer 3 | S=4 D=192 H=768
// ff0: w=ap_int<3> a=ap_int<4> impl=fabric
...
void ffn_block_3(float input[4][192], float output[4][192]) {

    const float s_act0 = 0.6437527537f;
    ap_int<4> x_int0[4][192];
    for (int si = 0; si < 4; si++) {
        Q_x_int0: for (int j = 0; j < 192; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)input[si][j] * 1.55339141f);
            _v = _v < -3.0f ? -3.0f : (_v > 3.0f ? 3.0f : _v);
            x_int0[si][j] = (ap_int<4>)_v;
        }
    }
    ...
    for (int s = 0; s < 4; s++) {
        for (int ii = 0; ii < 768; ii++) {
            ap_int<16> acc_int = 0;
            MAC_layers_3_feed_forward_0: for (int j = 0; j < 192; j++) {
        #pragma HLS PIPELINE II=1
    ...
    const float s_act1 = 1.2862312794f;
    ap_int<4> x_int1[4][768];
    for (int si = 0; si < 4; si++) {
        Q_x_int1: for (int j = 0; j < 768; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)mid[si][j] * 0.77746515f);
    ...
    for (int s = 0; s < 4; s++) {
        for (int ii = 0; ii < 192; ii++) {
            ap_int<17> acc_int = 0;
            MAC_layers_3_feed_forward_3: for (int j = 0; j < 768; j++) {
        #pragma HLS PIPELINE II=1
        #pragma HLS BIND_OP variable=acc_int op=mul impl=fabric
                acc_int += (ap_int<17>)layers_3_feed_forward_3_W[ii][j] * (ap_int<17>)x_int1[s][j];
            }
