// Delayed Scaling | Attention layer 3 | S=4 H=6 HD=32
...
void attention_layer_3(float input[4][192], float output[4][192]) {

    const float s_act_attn = 0.1832216978f;
    ap_int<4> x_int[4][192];
    for (int si = 0; si < 4; si++) {
        Q_x_int: for (int j = 0; j < 192; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)input[si][j] * 5.45786887f);
    ...
    for (int s = 0; s < 4; s++) {
        for (int ii = 0; ii < 576; ii++) {
            ap_int<16> acc_int = 0;
            MAC_layers_3_attention_in_proj: for (int j = 0; j < 192; j++) {
        #pragma HLS PIPELINE II=1
...
#pragma HLS PIPELINE II=1
    ...
    for (int s = 0; s < 4; s++) {
        for (int r = 0; r < 192; r++) {
            ...
            OUT_PROJ_3: for (int c = 0; c < 192; c++) {
#pragma HLS PIPELINE II=1
