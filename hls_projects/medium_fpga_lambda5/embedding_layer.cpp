// Delayed Scaling | Embedding | S=4 T=128 D=192
// w=ap_int<3> a=ap_int<6> acc=17b impl=fabric
// input: ap_fixed<16,2> shape [4][128]
// output: float shape [4][192]
#include "transformer_top.h"
#include "embedding.h"
#include "ap_int.h"

void embedding_layer(input_t input[4][TOKEN_DIM], float output[4][EMB_DIM]) {

    const float s_act_emb = 0.0071210838f;
    ap_int<6> x_int[4][128];
    for (int si = 0; si < 4; si++) {
        Q_x_int: for (int j = 0; j < 128; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)input[si][j] * 140.42806252f);
