...
void ffn_block_1(float input[4][128], float output[4][128]) {
    ...
    const float s_act0 = 0.3366946280f;
    ap_int<3> x_int0[4][128];
    for (int si = 0; si < 4; si++) {
        Q_x_int0: for (int j = 0; j < 128; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)input[si][j] * 2.97005036f);
    ...
    const float s_act1 = 0.3550616503f;
    ap_int<3> x_int1[4][512];
    for (int si = 0; si < 4; si++) {
        Q_x_int1: for (int j = 0; j < 512; j++) {
    #pragma HLS PIPELINE II=1
            float _v = roundf((float)mid[si][j] * 2.81641230f);
